...
// 차량 데이터는 "차량을 연동한 사람"의 계정 토큰으로 조회해야 하므로(다른 가족 구성원이
// 조회 요청을 하더라도) accountLink 자체를 기준으로 삼는다 — 요청자(request.user)가
// 아니다. 정확한 만료 임박 기준(여유 시간)은 실제 토큰 수명 확인 후 조정.
async function getValidAccessTokenFor(accountLink: HyundaiAccountLinkRow): Promise<string | null> {
...
async function getValidAccessTokenForUser(userId: string): Promise<string | null> {
...
async function getValidAccessTokenForVehicleLink(accountLinkId: string): Promise<string | null> {
