...
import { badgeDescKey, badgeIcon, badgeNameKey } from "../lib/badges";
...
export function LevelCard({ data }: { data: VehicleGamification }) {
  ...
  return (
    <section className="card">
      ...
      >
        {t("gamificationBadgesEarned", { count: data.earnedBadgeKeys.length, total: data.allBadgeKeys.length })}
        {" · "}
      ...
      {showBadges && (
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(80px, 1fr))", gap: 10, marginTop: 10 }}>
          {data.allBadgeKeys.map((key) => {
            const earned = data.earnedBadgeKeys.includes(key);
            const badgeKey = key as BadgeKey;
            return (
              <div
                key={key}
                title={t(badgeDescKey(badgeKey))}
                style={{
                  display: "flex",
