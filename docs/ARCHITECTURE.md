...
**알림**: 정비일·연료 부족 같은 알림은 웹 대시보드 내 배지/목록으로 확인하는 것이 기본이다. 실제로는 여기서 더 나아가 자체 PWA Web Push(VAPID)까지 구축했다 — 로그인 세션 없이도 브라우저/홈 화면 앱으로 알림이 온다(6절 참고).
...
- **외부 노출**: 실제로는 Caddy가 포트 80을 직접 열어 라우팅한다. Cloudflare Tunnel은 포트포워딩 없이 원격 접속하고 싶을 때 고려할 수 있는 옵션이지만 현재 도입 계획은 없다.
- **리버스 프록시**: Caddy — 설정이 짧다. 이미 Traefik을 쓰고 있다면 그걸 그대로 써도 무방하다.
...
- 알림의 1차 채널은 웹 대시보드다. 정비 임박·연료 부족 같은 항목은 로그인 시 배지/목록으로 바로 보인다.
- 실제로는 자체 PWA Web Push(VAPID)를 구축해 로그인 없이도 브라우저/홈 화면 앱으로 알림을 보낸다 — 초기 검토했던 "별도 푸시 인프라는 안 만들고 HA에 위임" 방향과 달리, 자체 푸시가 더 간단하다고 판단해 직접 구현했다(자세한 내용은 `docs/INTEGRATIONS.md`의 PWA 웹 푸시 참고).
- MQTT로는 원시 텔레메트리(위치/속도 등)만 발행한다. 계산된 리마인더·연료량을 MQTT discovery 형식으로 노출해 HA가 자동으로 센서 엔티티를 만들게 하는 기능은 검토했지만 구현·계획 둘 다 없다.
...
services:
  ...
  mosquitto:
    image: eclipse-mosquitto:2   # 선택 사항 — MQTT_URL을 설정할 때만 사용됨
  ...
  caddy:
    image: caddy:2
    ports: ["80:80"]      # 외부 요청을 직접 받아 api/frontend로 라우팅
...
3. **3단계**: 지도 API(네이버/카카오) 연동으로 트립 경로 시각화, 운전 습관 점수 계산.
4. **4단계**: Grafana 기반 비용/연비 분석 대시보드, 가족 구성원별 유류비 정산 리포트 고도화.

처음부터 4단계를 한 번에 설계하지 말고 1단계부터 실제로 써보면서 필요한 만큼만 확장하는 방식을 권한다. 특히 BullMQ+Redis는 "필요해지면 추가하는" 컴포넌트로 남겨두는 게 유지보수 부담을 줄인다. Traccar(전용 GPS/OBD 로거 연동)와 Cloudflare Tunnel(외부 접근)은 검토했지만 현재 도입 계획은 없다.
