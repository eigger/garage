...
- 워크스페이스 버전 `0.1.0 -> 0.2.0` 상향 (`package.json`, `apps/api`, `apps/web`, `packages/shared`)
- 브랜치: `release/v0.2.0-beta`
- 대규모 기능 통합 커밋 완료:
  - 카탈로그 기반 정비/행정 항목 key 저장 + i18n 단일 소스 정리
  - `PushSubscription.locale` 및 기기별 푸시 메시지 현지화
  - 지도 공급자(OSM/Kakao/Naver/T map) 선택형 트립 경로 표시 및 내비 딥링크
  - Opinet 연동, 빠른 입력, 트립 집계/표시 흐름 개선
- 베타 테스트 우선순위 UX 1~5 구현 완료:
  1. 리마인더에서 `확인함` 외 `스케줄`/`빠른입력` 바로가기
  2. 홈 대시보드 차량 요약 카드(현재 주행거리/최근 7일 주행/최근 주유/지남·임박 건수)
  3. 주유 이력의 구간 연비 표시(full-tank 간 `km/L`, `L/100km`)
  4. 차량 개요의 이번 달 비용 요약(주유/정비·행정/합계)
  5. 프리셋 변경의 기존 차량 일괄 반영 (`POST /api/maintenance-presets/apply-existing`)
- 빌드 검증:
  - `npm run build -w apps/api` 통과
  - `npm run build -w apps/web` 통과
...
- **오피넷 미연동 시 "주변 주유소 찾기" 버튼 숨김**: "오피넷 미연동 상태에서도 이 버튼이 계속 보이는가?"라는 사용자 질문에 코드를 확인하다가, 버튼이 연동 여부와 무관하게 항상 노출되고 키가 없으면 백엔드가 조용히 목(mock) 데이터 4곳(`하늘길 SK에너지 주유소` 등 그럴듯한 이름)으로 대체하고 있는 것을 발견함 — 프론트엔드가 이를 전혀 구분하지 않아서, 일반 사용자가 가짜 가격을 실제 가격인 줄 알고 저장할 위험이 있었음. 사용자가 "버튼을 아예 숨기는 방식"을 선택. 관리자 전용 `/api/settings`와 별도로 아무 로그인 사용자나 호출 가능한 `GET /api/opinet/configured`(`{ configured: boolean }`)를 신설(`apps/api/src/routes/opinet.ts`)하고, 빠른 입력 화면이 이 값을 확인해 미설정 시 버튼 자체를 렌더링하지 않도록 수정(`apps/web/app/vehicles/[id]/quick-log/page.tsx`). 수동 주유소명 입력란은 연동 여부와 무관하게 항상 노출되므로 수동 입력 경로는 그대로 유지됨. 실제 키로 켜짐/꺼짐 양쪽 다 브라우저에서 버튼 노출 여부를 확인.
